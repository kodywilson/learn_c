...
void draw_cartwheel(WINDOW *win) {
  ...
  frame_y = win_y - 3;
  frame_x = win_x - win_x - 6; // start at left outside window
...
void all_windows(WINDOW *stats_border, WINDOW *stats, WINDOW *game_text_border, WINDOW *game_text, WINDOW *select_border, WINDOW *select, WINDOW *input, int max_x) {
  ...
  mvwaddch(stats_border, stats_max_y - 1, stats_max_x / 4, '|');
  mvwaddch(stats_border, stats_max_y - 1, stats_max_x * 3 / 4, '|');
